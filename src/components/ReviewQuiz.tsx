    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="glass-panel-strong rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-5 border-b border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground">Review: {concept}</h3>
                <p className="text-xs text-muted-foreground">Current strength: {strength}%</p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-[rgba(255,255,255,0.1)] transition-colors">
              <div className="w-5 h-5">
                <LottieIcon name="x" size={20} />
              </div>
            </button>
          </div>
          ...
          <div className="p-5">
            ...
            {!started && (
              <div className="space-y-5 py-4">
                <p className="text-sm text-muted-foreground text-center">Choose how many questions you'd like</p>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={startQuiz}
                  className="w-full py-3 bg-gradient-to-r from-primary to-accent text-white rounded-lg font-medium"
                >
                  Start Quiz
                </motion.button>
              </div>
            ...
            {started && loading && (
              ...
              <div className="w-10 h-10">
                ...
                <span className="text-sm text-muted-foreground">Generating quiz…</span>
              </div>
            )}
            ...
            {started && !loading && questions.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <button onClick={onClose} className="mt-4 px-4 py-2 bg-primary/15 text-primary rounded-lg text-sm">
                  Close
                </button>
              </div>
            )}
            ...
            {started && !loading && !finished && question && (
              <div className="space-y-5">
                ...
                {/* Question */}
                <p className="text-foreground text-base leading-relaxed">{question.question}</p>
                ...
                <div className="space-y-2.5">
                  {question.options.map((opt, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span className="w-7 h-7 rounded-full border border-primary/25 flex items-center justify-center text-xs text-muted-foreground flex-shrink-0">
                          {String.fromCharCode(65 + idx)}
                        </span>
                        <span className="text-sm text-foreground">{opt}</span>
                        {answered && idx === question.correctIndex && (
                          <div className="w-5 h-5 ml-auto flex-shrink-0">
                  ...
                  <motion.div
                    ...
                    className={`p-3 rounded-lg border text-sm ${
                      isCorrect
                        ...
                        : 'bg-[rgba(255,77,109,0.05)] border-[rgba(255,77,109,0.2)] text-destructive'
                    }`}
                  >
                ...
                {answered && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={handleNext}
                    className="w-full py-3 bg-gradient-to-r from-primary to-accent text-white rounded-lg flex items-center justify-center gap-2"
                  >
                    {currentQ + 1 < questions.length ? (
                      <>
                        Next Question 
            ...
            {finished && (
              ...
              >
                ...
                <div>
                  <h4 className="text-xl font-semibold text-foreground mb-1">Review Complete!</h4>
                ...
                {submitting ? (
                  ...
                  <div className="bg-[rgba(0,255,163,0.1)] border border-[rgba(0,255,163,0.2)] rounded-lg p-4">
                    <p className="text-sm text-muted-foreground mb-1">Updated Memory Strength</p>
                    <div className="flex items-center justify-center gap-3">
                      <span className="text-muted-foreground line-through">{strength}%</span>
                      <div className="w-4 h-4">
                        <LottieIcon name="arrowRight" size={16} />
                      </div>
                ...
                <div className="flex gap-3">
                  <motion.button
                    ...
                    className="flex-1 py-3 border border-primary/25 text-primary rounded-lg flex items-center justify-center gap-2"
                  >
                    <div className="w-4 h-4">
                      <LottieIcon name="refresh" size={16} />
                    </div> Retry
                  </motion.button>
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={onClose}
                    className="flex-1 py-3 bg-gradient-to-r from-primary to-accent text-white rounded-lg"
                  >
                    Done
                  </motion.button>
                </div>
